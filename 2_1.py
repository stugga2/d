...
kp_yaw = 1.8                # угловой коэффициент
...
for step in range(3000):
    ...
    if step % 250 == 0:
        ...
        _, _, yaw = p.getEulerFromQuaternion(orn)
        print(f"{step:4d} | ({pos[0]:.2f}, {pos[1]:.2f}) | {math.degrees(yaw):.1f}°")
...
plt.title('Траектория движения модели омниробота в плоскости XY', fontsize=14)
...
plt.title('Изменение ошибки позиции', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
